document.querySelectorAll('.thumb').forEach(b => {
    b.onclick = () => {
        ...
        h.lastElementChild.innerText = b.innerText;
        h.firstElementChild.src = b.firstElementChild.src;
    }
})

/*  codigo gerado por ai para transição de paginas
    document.addEventListener('click', e => {
        if (e.target.matches('a')) {
            e.preventDefault();
            const url = e.target.href;

            // Iniciar uma animação de saída
            document.body.classList.add('page-exit');

            // Carregar o novo conteúdo e inseri-lo no DOM
            fetch(url)
            ...
            .then(html => {
                ...
                // Iniciar uma animação de entrada
